    ...
    private boolean checkValidEnrolment(Student student) {
        ...
        return prereqs.stream()
                .allMatch(prereq -> student.getEnrolments().stream()
                        .anyMatch(enrolment -> enrolment.getCourse().equals(prereq) && enrolment.getGrade() != null
                                && enrolment.getStudentMark() >= 50 && !enrolment.getStudentGrade().equals("FL")
                                && !enrolment.getStudentGrade().equals("UF")));
